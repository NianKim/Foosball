...
LAT_PASS_PENALTY = 0.53  # penalty per metre of lateral distance when choosing pass target
...
# PRE: ball_coords is a 2D point. target is a 2D point. opp_coords is an array of opponent 2D points.
# POST: returns a score representing how clear the passing lane is from ball_coords to target, considering the positions of opponents in opp_coords. Higher scores indicate clearer lanes.
def pass_lane_clearance(ball_coords: np.ndarray,
                        target: np.ndarray,
                        opp_coords: np.ndarray) -> float:
    line_vec = target - ball_coords
    ...
    for opp in opp_coords:
        diff = opp - ball_coords
        proj = np.dot(diff, line_dir)
        if proj <= 0:
            d = np.linalg.norm(opp - ball_coords)
        elif proj >= line_len:
            d = np.linalg.norm(opp - target)
        else:
            d = np.linalg.norm(diff - proj * line_dir)
        closest = min(closest, d)
    return closest

# PRE: target is a 2D point, coord is a 2D point ball_coords 2D point, opp_coords is an array of opponent positions. sign is +1 for team 0 and -1 for team 1, used to determine forward direction.
# POST: returns a score for how good a pass to the target would be, considering the current position of the player (coord), the ball, and the opponents. Higher scores indicate better pass targets.
#       The score is based on forward progress towards the opponent's goal, lateral distance from the ball, movement towards the target, clearance from opponents, and pressure from nearby opponents.
def pass_target_score(target: np.ndarray, coord: np.ndarray,
                      ball_coords: np.ndarray, opp_coords: np.ndarray,
                      sign: float) -> float:
    forward_component = target[0] * sign
    lateral_penalty = abs(target[1] - ball_coords[1]) * LAT_PASS_PENALTY
    forward_movement = np.clip((target[0] - coord[0]) * sign, 0.0, 10.0)
    movement_bonus = forward_movement * 0.12
    clearance = pass_lane_clearance(ball_coords, target, opp_coords)
    safety_bonus = 0.08 * np.clip(clearance, 0.0, 12.0)
    rough_target_proximity = np.min(np.linalg.norm(opp_coords - target, axis=1))
    pressure_penalty = 0.0
    if rough_target_proximity < 3.5:
        pressure_penalty = (3.5 - rough_target_proximity) * 0.6
    return forward_component - lateral_penalty + movement_bonus + safety_bonus - pressure_penalty
...
def ball_carrier_action(ball_coords: np.ndarray,
                        final_coords: np.ndarray,
                        orig_coords: np.ndarray,
                        own_team: int,
                        opp_coords: np.ndarray) -> np.ndarray:
    ...
    # Priority 2: pass to the best forward-safety balance in legal range
    #             skip any teammate whose position would score an own goal
    in_range = []
    for final_coord, orig_coord in zip(final_coords, orig_coords):
        d = np.linalg.norm(final_coord - ball_coords)
        if MIN_PASS < d < MAX_PASS:
            if (final_coord[0] - own_goal_x) * sign < 0:
                continue                        # would land behind own goal — skip
            score = pass_target_score(final_coord, orig_coord, ball_coords, opp_coords, sign)
            in_range.append((score, final_coord))
    if in_range:
...
def gradient_strategy(strat_input: StrategyInput) -> StrategyOutput:
    ...
    for i in order:
        ...
        if ball.team == own_team and i == carrier_idx:
            new_coords[i] = ball_carrier_action(ball_coords, new_coords, own_coords, own_team, opp_coords)
            continue
